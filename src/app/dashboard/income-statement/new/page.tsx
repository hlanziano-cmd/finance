...
import { ArrowLeft, TrendingUp, TrendingDown, AlertCircle, Info, Lightbulb, HelpCircle, Download } from 'lucide-react';
import { Card, CardHeader, CardTitle, CardDescription, CardContent } from '@/src/components/ui/Card';
import { Button } from '@/src/components/ui/Button';
import { LabelWithTooltip } from '@/src/components/ui/Tooltip';
import { CurrencyInput } from '@/src/components/ui/CurrencyInput';
import { useCreateIncomeStatement } from '@/src/lib/hooks/useIncomeStatement';
...
export default function NewIncomeStatementPage() {
  ...
  const [taxRate, setTaxRate] = useState(35); // Tasa de impuestos en Colombia
  const [depreciationAmortization, setDepreciationAmortization] = useState(0); // Depreciación y amortización
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error('Error al crear estado de resultados:', error);
    }
  };
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Nuevo Estado de Resultados</h1>
          <p className="mt-1 text-sm text-gray-500">
            Registra tus ingresos y gastos del período
          </p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex items-center justify-end gap-3">
          ...
          <Button
            type="submit"
            disabled={ingresos === 0 || createMutation.isPending}
          >
            {createMutation.isPending ? 'Guardando...' : 'Guardar Estado de Resultados'}
          </Button>
        </div>
      </form>
    </div>
